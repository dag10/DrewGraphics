...
std::unique_ptr<dg::PointShadowScene> dg::PointShadowScene::Make() {
  return std::unique_ptr<dg::PointShadowScene>(new dg::PointShadowScene(false));
}

std::unique_ptr<dg::PointShadowScene> dg::PointShadowScene::MakeVR() {
  return std::unique_ptr<dg::PointShadowScene>(new dg::PointShadowScene(true));
}

dg::PointShadowScene::PointShadowScene(bool enableVR) : Scene() {
  vr.requested = enableVR;
}

void dg::PointShadowScene::Initialize() {
  Scene::Initialize();
  ...
  auto pointLight = std::make_shared<PointLight>(
    glm::vec3(0.5f, 0.63f, 0.76f), 1.43f, 1.132f, 3.1f);
  ...
  AddChild(std::make_shared<Model>(
      Mesh::Cube, cubeMaterial,
      Transform::TRS({-0.3, cubeSize * 0.5f, -1.2},
                     glm::quat(glm::radians(glm::vec3(0, 3, 0))),
                     glm::vec3(cubeSize))));
  ...
  auto roomSides = std::make_shared<SceneObject>(Transform::T({ -6, 0, -6 }));
  AddChild(roomSides);
...
void dg::PointShadowScene::Update() {
  Scene::Update();
...
void dg::PointShadowScene::ClearBuffer() {
