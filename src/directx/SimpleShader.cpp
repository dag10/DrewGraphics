...
// --------------------------------------------------------
ISimpleShader::ISimpleShader(ID3D11Device* device,
  ...
  if (this->device == nullptr) {
    this->device = Graphics::Instance->device;
  }

  if (this->deviceContext == nullptr) {
    this->deviceContext = Graphics::Instance->context;
  }
...
bool ISimpleShader::LoadShaderFile(LPCWSTR shaderFile) {
  ...
  for (unsigned int r = 0; r < resourceCount; r++) {
    ...
    switch (resourceDesc.Type) {
      case D3D_SIT_TEXTURE:  // A texture resource
      {
        ...
        textureTable.insert(
            std::pair<std::string, SimpleSRV*>(resourceDesc.Name, srv));
      ...
      case D3D_SIT_SAMPLER:  // A sampler resource
      {
        ...
        samplerTable.insert(
            std::pair<std::string, SimpleSampler*>(resourceDesc.Name, samp));
  ...
  for (unsigned int b = 0; b < constantBufferCount; b++) {
    ...
    cbTable.insert(std::pair<std::string, SimpleConstantBuffer*>(
        bufferDesc.Name, &constantBuffers[b]));
    ...
    for (unsigned int v = 0; v < bufferDesc.Variables; v++) {
      ...
      varTable.insert(
          std::pair<std::string, SimpleShaderVariable>(varName, varStruct));
...
SimpleConstantBuffer* ISimpleShader::FindConstantBuffer(std::string name) {
  ...
  // Did we find the key?
  if (result == cbTable.end()) return 0;
...
void ISimpleShader::CopyAllBufferData() {
  ...
  for (unsigned int i = 0; i < constantBufferCount; i++) {
    ...
    deviceContext->UpdateSubresource(constantBuffers[i].ConstantBuffer, 0, 0,
...
void ISimpleShader::CopyBufferData(unsigned int index) {
  ...
  // Validate the index
  if (index >= this->constantBufferCount) return;
  ...
  deviceContext->UpdateSubresource(cb->ConstantBuffer, 0, 0,
...
// --------------------------------------------------------
bool ISimpleShader::SetData(std::string name, const void* data,
                            unsigned int size) {
  ...
  memcpy(constantBuffers[var->ConstantBufferIndex].LocalDataBuffer +
             var->ByteOffset,
         data, size);
...
// Sets a MATRIX (4x4) variable by name in the local data buffer
// --------------------------------------------------------
bool ISimpleShader::SetMatrix4x4(std::string name,
                                 const DirectX::XMFLOAT4X4 data) {
...
const SimpleSRV* ISimpleShader::GetShaderResourceViewInfo(std::string name) {
  ...
  // Did we find the key?
  if (result == textureTable.end()) return 0;
...
const SimpleSampler* ISimpleShader::GetSamplerInfo(std::string name) {
  ...
  // Did we find the key?
  if (result == samplerTable.end()) return 0;
...
unsigned int ISimpleShader::GetBufferSize(unsigned int index) {
  // Valid index?
  if (index >= constantBufferCount) return -1;
...
// --------------------------------------------------------
SimpleVertexShader::SimpleVertexShader(ID3D11Device* device,
                                       ID3D11DeviceContext* context)
...
bool SimpleVertexShader::CreateShader(ID3DBlob* shaderBlob) {
  ...
  HRESULT result = device->CreateVertexShader(
      shaderBlob->GetBufferPointer(), shaderBlob->GetBufferSize(), 0, &shader);
  ...
  for (unsigned int i = 0; i < shaderDesc.InputParameters; i++) {
    ...
    if (isPerInstance) {
      elementDesc.InputSlot =
          1;  // Assume per instance data comes from another input slot!
  ...
  HRESULT hr = device->CreateInputLayout(
      &inputLayoutDesc[0], (unsigned int)inputLayoutDesc.size(),
...
// --------------------------------------------------------
bool SimpleVertexShader::SetShaderResourceView(std::string name,
                                               ID3D11ShaderResourceView* srv) {
  // Look for the variable and verify
  const SimpleSRV* srvInfo = GetShaderResourceViewInfo(name);
  if (srvInfo == 0) return false;
...
// --------------------------------------------------------
bool SimpleVertexShader::SetSamplerState(std::string name,
                                         ID3D11SamplerState* samplerState) {
  // Look for the variable and verify
  const SimpleSampler* sampInfo = GetSamplerInfo(name);
  if (sampInfo == 0) return false;
...
// Constructor just calls the base
// --------------------------------------------------------
SimplePixelShader::SimplePixelShader(ID3D11Device* device,
                                     ID3D11DeviceContext* context)
    : ISimpleShader(device, context) {
  this->shader = 0;
}
...
// Returns true if a texture of the given name was found, false otherwise
// --------------------------------------------------------
bool SimplePixelShader::SetShaderResourceView(std::string name,
                                              ID3D11ShaderResourceView* srv) {
  // Look for the variable and verify
  const SimpleSRV* srvInfo = GetShaderResourceViewInfo(name);
  if (srvInfo == 0) return false;
...
// Returns true if a sampler of the given name was found, false otherwise
// --------------------------------------------------------
bool SimplePixelShader::SetSamplerState(std::string name,
                                        ID3D11SamplerState* samplerState) {
  // Look for the variable and verify
  const SimpleSampler* sampInfo = GetSamplerInfo(name);
  if (sampInfo == 0) return false;
...
// Constructor just calls the base
// --------------------------------------------------------
SimpleDomainShader::SimpleDomainShader(ID3D11Device* device,
                                       ID3D11DeviceContext* context)
    : ISimpleShader(device, context) {
  this->shader = 0;
}
...
void SimpleDomainShader::SetShaderAndCBs() {
  ...
  // Set the constant buffers
  for (unsigned int i = 0; i < constantBufferCount; i++) {
    deviceContext->DSSetConstantBuffers(constantBuffers[i].BindIndex, 1,
                                        &constantBuffers[i].ConstantBuffer);
  }
}
...
// Returns true if a texture of the given name was found, false otherwise
// --------------------------------------------------------
bool SimpleDomainShader::SetShaderResourceView(std::string name,
                                               ID3D11ShaderResourceView* srv) {
  // Look for the variable and verify
  const SimpleSRV* srvInfo = GetShaderResourceViewInfo(name);
  if (srvInfo == 0) return false;
...
// Returns true if a sampler of the given name was found, false otherwise
// --------------------------------------------------------
bool SimpleDomainShader::SetSamplerState(std::string name,
                                         ID3D11SamplerState* samplerState) {
  // Look for the variable and verify
  const SimpleSampler* sampInfo = GetSamplerInfo(name);
  if (sampInfo == 0) return false;
...
// Constructor just calls the base
// --------------------------------------------------------
SimpleHullShader::SimpleHullShader(ID3D11Device* device,
                                   ID3D11DeviceContext* context)
    : ISimpleShader(device, context) {
  this->shader = 0;
}
...
void SimpleHullShader::SetShaderAndCBs() {
  ...
  // Set the constant buffers?
  for (unsigned int i = 0; i < constantBufferCount; i++) {
    deviceContext->HSSetConstantBuffers(constantBuffers[i].BindIndex, 1,
                                        &constantBuffers[i].ConstantBuffer);
  }
}
...
// Returns true if a texture of the given name was found, false otherwise
// --------------------------------------------------------
bool SimpleHullShader::SetShaderResourceView(std::string name,
                                             ID3D11ShaderResourceView* srv) {
  // Look for the variable and verify
  const SimpleSRV* srvInfo = GetShaderResourceViewInfo(name);
  if (srvInfo == 0) return false;
...
// Returns true if a sampler of the given name was found, false otherwise
// --------------------------------------------------------
bool SimpleHullShader::SetSamplerState(std::string name,
                                       ID3D11SamplerState* samplerState) {
  // Look for the variable and verify
  const SimpleSampler* sampInfo = GetSamplerInfo(name);
  if (sampInfo == 0) return false;
...
// --------------------------------------------------------
SimpleGeometryShader::SimpleGeometryShader(ID3D11Device* device,
                                           ID3D11DeviceContext* context,
                                           bool useStreamOut,
...
bool SimpleGeometryShader::CreateShader(ID3DBlob* shaderBlob) {
  ...
  // Using stream out?
  if (useStreamOut) return this->CreateShaderWithStreamOut(shaderBlob);
...
bool SimpleGeometryShader::CreateShaderWithStreamOut(ID3DBlob* shaderBlob) {
  ...
  unsigned int rast =
      allowStreamOutRasterization ? 0 : D3D11_SO_NO_RASTERIZED_STREAM;
...
// --------------------------------------------------------
bool SimpleGeometryShader::CreateCompatibleStreamOutBuffer(
    ID3D11Buffer** buffer, int vertexCount) {
...
// --------------------------------------------------------
void SimpleGeometryShader::UnbindStreamOutStage(
...
// Returns true if a texture of the given name was found, false otherwise
// --------------------------------------------------------
bool SimpleGeometryShader::SetShaderResourceView(
    std::string name, ID3D11ShaderResourceView* srv) {
  // Look for the variable and verify
  const SimpleSRV* srvInfo = GetShaderResourceViewInfo(name);
  if (srvInfo == 0) return false;
...
// Returns true if a sampler of the given name was found, false otherwise
// --------------------------------------------------------
bool SimpleGeometryShader::SetSamplerState(std::string name,
                                           ID3D11SamplerState* samplerState) {
  // Look for the variable and verify
  const SimpleSampler* sampInfo = GetSamplerInfo(name);
  if (sampInfo == 0) return false;
...
// Constructor just calls the base
// --------------------------------------------------------
SimpleComputeShader::SimpleComputeShader(ID3D11Device* device,
                                         ID3D11DeviceContext* context)
    : ISimpleShader(device, context) {
  this->shader = 0;
}
  ...
  for (unsigned int r = 0; r < resourceCount; r++) {
    ...
    switch (resourceDesc.Type) {
      ...
      case D3D_SIT_UAV_RWTYPED:
        uavTable.insert(std::pair<std::string, unsigned int>(
...
void SimpleComputeShader::SetShaderAndCBs() {
  ...
  // Set the constant buffers?
  for (unsigned int i = 0; i < constantBufferCount; i++) {
    deviceContext->CSSetConstantBuffers(constantBuffers[i].BindIndex, 1,
                                        &constantBuffers[i].ConstantBuffer);
  }
}
...
// --------------------------------------------------------
void SimpleComputeShader::DispatchByGroups(unsigned int groupsX,
...
// --------------------------------------------------------
void SimpleComputeShader::DispatchByThreads(unsigned int threadsX,
...
// Returns true if a texture of the given name was found, false otherwise
// --------------------------------------------------------
bool SimpleComputeShader::SetShaderResourceView(std::string name,
                                                ID3D11ShaderResourceView* srv) {
  // Look for the variable and verify
  const SimpleSRV* srvInfo = GetShaderResourceViewInfo(name);
  if (srvInfo == 0) return false;
...
// Returns true if a sampler of the given name was found, false otherwise
// --------------------------------------------------------
bool SimpleComputeShader::SetSamplerState(std::string name,
                                          ID3D11SamplerState* samplerState) {
  // Look for the variable and verify
  const SimpleSampler* sampInfo = GetSamplerInfo(name);
  if (sampInfo == 0) return false;
...
// --------------------------------------------------------
bool SimpleComputeShader::SetUnorderedAccessView(
    std::string name, ID3D11UnorderedAccessView* uav,
    unsigned int appendConsumeOffset) {
  // Look for the variable and verify
  unsigned int bindIndex = GetUnorderedAccessViewIndex(name);
  if (bindIndex == -1) return false;

  // Set the shader resource view
  deviceContext->CSSetUnorderedAccessViews(bindIndex, 1, &uav,
                                           &appendConsumeOffset);
...
int SimpleComputeShader::GetUnorderedAccessViewIndex(std::string name) {
  ...
  // Did we find the key?
  if (result == uavTable.end()) return -1;
